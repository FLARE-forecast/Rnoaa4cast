...
noaa_gefs_grid_download <- function(lat_list, lon_list, forecast_time, forecast_date ,model_name_raw, output_directory) {
  ...
  download_grid <- function(ens_index, location, directory, hours_char, cycle, base_filename1, vars,working_directory){
    ...
    if(ens_index == 1){
      ...
      base_filename2 <- paste0("flxf",curr_date, hour,.01, ,".t",start_date, cycle,".grb2")
...
#URL= https://nomads.ncep.noaa.gov/cgi-bin/filter_cfs_flx.pl?file=flxf2021072600.01.2021072600.grb2&lev_10_m_above_ground=on&lev_2_m_above_ground=on&lev_entire_atmosphere_%5C%28considered_as_a_single_layer%5C%29=on&lev_surface=on&var_DLWRF=on&var_DSWRF=on&var_PRATE=on&var_PRES=on&var_SPFH=on&var_TMP=on&var_UGRD=on&var_VGRD=on&subregion=&leftlon=0&rightlon=360&toplat=90&bottomlat=-90&dir=%2Fcfs.20210726%2F00%2F6hrly_grib_01
      ...
      for(j in 1:length(forecast_hours)){
        ...
        if(is.na(forecast_time) | forecast_hours[j] %in% as.numeric(forecast_time)){
          ...
          if(new_download){
            ...
            directory <- paste0("&dir=%2Fcfs.",curr_date,"%2F",cycle,"%2F6hrly_grib_01")
